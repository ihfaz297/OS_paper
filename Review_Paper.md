...
- **Observations:** The **Normalized Turnaround Time (NTT)** and **Predictability CV** metrics reveal that while SJF is efficient on average, it is highly unpredictable for long-running tasks. **Round Robin** provides the most consistent experience for small-to-medium tasks. The **Convoy Effect** measure shows that FCFS and even RR (at certain quanta) can suffer from blockage where short tasks are significantly delayed by earlier long-running processes.

### Size-Based Fairness and Resource Equity
![Size Fairness Ratio](images/Size_Fairness_Ratio.png)
- **Observations:** The **Size Fairness Ratio** highlights a critical bias in SJF and RR. SJF achieves its low AWT by massively prioritizing small jobs, leading to a ratio near 0.13 (extreme bias). **Round Robin** also favors smaller jobs but more moderately. **FCFS** and **Priority with Aging** maintain the highest size-based equity, treating jobs of different scales more uniformly, though at the cost of overall system throughput.
...
FCFS forces severe "mathematical fairness" under these highly skewed historical workloads simply by enforcing a brutal convoy; equal suffering results in a high mathematical fairness score but a catastrophic Maximum Bounded Slowdown (over 1,000,000x penalty) and immense active memory congestion in the Ready Queue. SJF achieves absolute optimal Average Waiting Time but relies on structural starvation that ruins its JFI score and drives its Wait Time Coefficient of Variation into erratic territory for long-running tasks. Priority scheduling aligns strictly to organizational hierarchies, but simple linear Aging mechanics, while preventing indefinite starvation, are often overwhelmed by the sheer burst velocity of real-world supercomputers, resulting in practical starvation rates exceeding 90%.

Round Robin proves to be the ultimate practical synthesis for bursty, real-world datasets exactly like the SDSC, KTH, and CEA-Curie historical archives. It aggressively drops maximum responsiveness latencies, controls Bounded Slowdown for small tasks, massively alleviates Ready Queue pressure, and ensures better predictability for interactive workloads. However, the introduction of advanced metrics like **Convoy Effect** and **Size Fairness Ratio** reveals that even RR is not a silver bullet; its bias toward short jobs must be carefully balanced against the needs of large-scale computational tasks. For highly variant empirical HPC queue patterns, periodic preemption acts as an indispensable equalizer, provided the quantum is tuned to the specific workload distribution.
